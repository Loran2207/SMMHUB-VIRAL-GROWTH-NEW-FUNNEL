...
function Emblem() {
  return <span className="grid size-[26px] place-items-center rounded-[8px] bg-accent"><Play size={13} fill="white" strokeWidth={0} className="ml-0.5 text-white" /></span>;
}

export function FeaturedIcon({ children }: { children: ReactNode }) {
  return <span className="grid size-[28px] shrink-0 place-items-center rounded-[8px] border border-[#ebebee] text-ink-soft shadow-[0px_1px_2px_0px_rgba(16,24,40,0.05)]">{children}</span>;
}

export function TopCard({ cols }: { cols: { icon: ReactNode; label: string; value: string }[] }) {
  return (
    <div className="px-4 pt-6">
      <div className="mb-2 flex">{cols.map((c, i) => <div key={i} className="flex flex-1 justify-center text-accent">{c.icon}</div>)}</div>
      <div className="flex rounded-[12px] border border-[#eaecf0] bg-white py-2 shadow-[0px_4px_8px_-2px_rgba(16,24,40,0.1),0px_2px_4px_-2px_rgba(16,24,40,0.06)]">
        {cols.map((c, i) => (
