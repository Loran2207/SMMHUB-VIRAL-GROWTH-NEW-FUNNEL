...
import { StatusBar, HomeIndicator } from "@/funnel/components/Chrome";

function FullBleed({ img, children }: { img: string; children: ReactNode }) {
  return (
    <PhoneFrame>
      <div className="relative h-dvh w-full overflow-hidden bg-black">
        <img src={img} alt="" className="absolute inset-0 size-full object-cover" />
        <div className="absolute inset-x-0 top-0 z-10"><StatusBar dark /></div>
        <div className="relative z-10 flex h-full flex-col">{children}</div>
        <div className="absolute inset-x-0 bottom-0 z-10"><HomeIndicator dark /></div>
      </div>
    </PhoneFrame>
  );
}
...
export function Welcome1({ onNext }: { onNext: () => void }) {
  return (
    <FullBleed img="/img/welcome-woman.png">
      <div onClick={onNext} className="mt-auto flex cursor-pointer flex-col gap-3 bg-gradient-to-t from-[#231a12] via-[#231a12]/85 to-transparent px-5 pb-7 pt-28 text-white">
        <WelcomeHead />
...
export function Welcome2({ onNext }: { onNext: () => void }) {
  return (
    <FullBleed img="/img/welcome-woman.png">
      <div className="mt-auto flex flex-col gap-3 bg-gradient-to-t from-[#231a12] via-[#231a12]/85 to-transparent px-5 pb-6 pt-24 text-white">
        <WelcomeHead />
        <div className={CARD}>I&apos;m Sandy, your <b>Social Media Coach</b> - here to help you <b>grow your audience with confidence.</b></div>
...
export function LoggedIn({ onNext }: { onNext: () => void }) {
  return (
    <FullBleed img="/img/loggedin-room.png">
      <div className="flex h-full flex-col items-center justify-center px-6 text-center text-white">
        <div className="grid size-20 place-items-center rounded-full text-[64px] leading-none">&#10003;</div>
